...
draftReview=async function(id){
  ...
  const reviewRecommendation=draft.quality_review_recommendation||{level:'low',recommended:false,reasons:[]};
  ...
  const reviewGuidance=review.status==='not_run'?(reviewRecommendation.recommended?`Quality review is recommended before approval because this draft includes ${esc((reviewRecommendation.reasons||[]).join(', ')||'higher-risk technical content')}. You can still approve after validation.`:`Review risk: <strong>Low</strong>. Quality review is optional; you may approve after local validation or run review first.`):review.status==='failed'?`Quality review failed. You may approve after local validation or retry review.`:`Generate Draft has already been paid; review sends this existing draft to one quality-review call only.`;
  const approveLabel=review.status==='not_run'?(reviewRecommendation.recommended?'Approve anyway':'Approve'):'Validate & approve';
  app.innerHTML=`<section class="form"><div class="eyebrow">AI draft · human approval required</div><h1>${esc(draft.title)}</h1><p class="notice">Quality review: <strong>${esc(reviewLabel)}</strong>. ${reviewGuidance}</p><div id="draft-status" class="status" aria-live="polite"></div><div id="approval-confirmation" class="card" hidden></div><form id="draft-form"><div class="field-grid"><label>Title<input data-simple-field="title" value="${esc(payload.title)}"></label><label>Category<select data-simple-field="category">${categoryOptions(payload.category)}</select></label><label>Difficulty<select data-simple-field="difficulty">${['beginner','intermediate','advanced'].map(value=>`<option value="${value}" ${payload.difficulty===value?'selected':''}>${title(value)}</option>`).join('')}</select></label><label>Tags<input id="simple-tags" value="${esc((payload.tags||[]).join(', '))}"></label>${simpleTopicFields.map(field=>`<label class="wide">${title(field)}<textarea data-simple-field="${field}">${esc(payload[field]||'')}</textarea></label>`).join('')}<label class="wide">Mathematical foundation overview<textarea id="simple-math-overview">${esc(payload.mathematical_foundation?.overview||'')}</textarea></label></div><div class="actions"><button type="button" id="simple-save" class="secondary">Save edits</button><button type="button" id="simple-approve">${approveLabel}</button><button type="button" id="simple-discard" class="danger">Discard</button></div></form><details><summary>Advanced JSON editor</summary><p class="muted">Apply JSON locally, then save. Topic IDs remain backend-owned on approval.</p><textarea id="simple-json" class="draft-editor">${esc(JSON.stringify(payload,null,2))}</textarea><button type="button" id="simple-apply" class="secondary">Apply advanced JSON</button></details><section class="card"><h2>Quality review</h2><p class="notice">Review maximum: ${money(estimate.maximum_estimated_cost_usd)} · remaining: ${money(estimate.remaining_budget_usd)}</p><button type="button" id="simple-quality" ${review.status==='running'?'disabled':''}>${review.status==='reviewed'?'Run explicit re-review':'Run quality review'}</button>${revisions.revisions.length?`<p class="muted">${revisions.revisions.length} saved quality-review revision(s) are available for recovery.</p>`:''}</section></section>`;
  ...
  const approve=async()=>{try{const approved=await runUiAction({key:`draft:${id}:mutation`,button:approveButton,loadingLabel:'Approving…',status,startedMessage:dirty?'Saving changes before approval…':'Validating draft…',scope:'DRAFT UI',conflicts:[saveButton,qualityButton,discardButton],action:async()=>{if(dirty)await persist();const validation=await api('/ai/drafts/'+id+'/validate');if(!validation.valid)throw new Error(validation.errors.map(item=>`${item.field}: ${item.message}`).join(' '));return request('/ai/drafts/'+id+'/approve');}});if(!approved?.ignored)location.hash='#/questions/create/'+slug(payload.title);}catch(_){}};
  approveButton.onclick=()=>{
    if(review.status!=='not_run')return approve();
    const confirmation=document.querySelector('#approval-confirmation');
    if(!confirmation.hidden)return;
    confirmation.hidden=false;
    confirmation.innerHTML=`<p>This topic has not been quality-reviewed yet. You can approve it now or run Quality Review first.</p><div class="actions"><button type="button" id="confirm-approve">Approve anyway</button><button type="button" id="confirm-quality" class="secondary">Run Quality Review first</button></div>`;
    document.querySelector('#confirm-approve').onclick=approve;
    document.querySelector('#confirm-quality').onclick=()=>qualityButton.click();
  };
